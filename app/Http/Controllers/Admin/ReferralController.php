...
class ReferralController extends Controller
{
    ...
    public function commission($user = '')
    {
        ...
        $commission = RefferedCommission::query();

        if($user){
            $commission->where('reffered_by', $user->id);
        }
        
        $commission = $commission->latest()->paginate();

        $pageTitle = 'Commission Log';

        return view('backend.report.commission',compact('commission','pageTitle'));
    }
}
