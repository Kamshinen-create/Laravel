...
class UserController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $commission = RefferedCommission::where('reffered_by', Auth::id())->sum('amount');
        ...
        return view($this->template . 'user.dashboard', compact('commission', 'pageTitle', 'interestLogs', 'totalInvest', 'currentInvest', 'currentPlan', 'allPlan', 'withdraw', 'pendingInvest', 'pendingWithdraw', 'totalDeposit'));
    }
    ...
    public function profileUpdate(Request $request)
    {
        $request->validate([
            ...
            'username' => 'required|string|max:255|unique:users,username,' . Auth::id() . '|regex:/^[a-zA-Z0-9_]+$/',
        ...
        if ($request->hasFile('image')) {
            try {
                $filename = uploadImage($request->image, commonPath('user'), $user->image);
                $user->image = $filename;
            } catch (\Exception $e) {
    ...
    public function withdrawCompleted(Request $request)
    {
        ...
        try {
            ...
            $notify[] = ['error', 'Withdraw failed. Please try again.'];
            return back()->withNotify($notify);
        }
    }
    ...
    public function commission(Request $request)
    {
        ...
        $commission = RefferedCommission::when($request->date, function ($item) use ($request) {
            $item->whereDate('created_at', $request->date);
        })->where('reffered_by', Auth::id())->latest()->paginate(10, ['*'], 'commission');

        return view($this->template . 'user.commision_log', compact('pageTitle', 'commission'));
    }
    ...
    public function transferMoney(Request $request)
    {
        ...
        $commission = $general->trans_type === 'percent' ? ($request->amount * $general->trans_charge) / 100 :  $general->trans_charge;

        $totalSendAmount = $commission + $request->amount;
    ...
    public function kycUpdate(Request $request)
    {
        ...
        foreach ($data as $key => $upload) {
            ...
            if ($request->hasFile($key)) {
                ...
                $filename = uploadImage($upload, commonPath('user'));
    ...
    public function planRemove(Request $request, $id)
    {
        ...
        try {
            ...
            return back()->with('error', 'Failed to unsubscribe from plan. Please try again.');
        }
    }
}
